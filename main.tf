...
resource "aws_subnet" "sathya_subnet" {
  ...
  tags = {
    Name = "sathya_subnetsubnet-${count.index}"
  }
}
...
resource "aws_eks_node_group" "sathya" {
  ...
  remote_access {
    ...
    source_security_group_ids = [aws_security_group.sathya_node_sg.id]
  }
}
...
resource "aws_iam_role_policy_attachment" "sathya_cluster_role_policy" {
...
resource "aws_iam_role_policy_attachment" "sathya_node_group_role_policy" {
  role       = aws_iam_role.sathya_node_group_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy"
}

resource "aws_iam_role_policy_attachment" "sathya_node_group_cni_policy" {
  role       = aws_iam_role.sathya_node_group_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy"
}

resource "aws_iam_role_policy_attachment" "sathya_node_group_registry_policy" {
  role       = aws_iam_role.sathya_node_group_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryReadOnly"
}

resource "aws_iam_role_policy_attachment" "sathya_node_group_ebs_policy" {
  role       = aws_iam_role.sathya_node_group_role.name
